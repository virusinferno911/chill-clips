...
function AppContent() {
  ...
  const renderPage = () => {
    ...
    if (search.trim()) {
      return (
        <section style={{ paddingBottom: 80, paddingTop: 100 }}>
          <MovieRow 
    ...
    return (
      <>
          ...
          {[['24+', 'Movies & Shows'], ['6', 'Genres'], ['Free', 'Trailers'], ['HD', 'Quality']].map(([val, label]) => (
            <div key={label} style={{ textAlign: 'center' }}>
              <div style={{
                ...
                fontSize: 'clamp(22px,4vw,28px)', color: 'var(--gold)', letterSpacing: 1,
              }}>{val}</div>
        ...
        <section style={{ paddingBottom: 80, paddingTop: 40 }}>
          <MovieRow title="Trending Now" movies={TRENDING} {...sharedProps} />
          {Object.entries(BY_GENRE)
            .filter(([, v]) => v.length > 0)
            .map(([genre, list]) => (
              <MovieRow key={genre} title={genre} movies={list} {...sharedProps} />
          ))}
        </section>
      </>
    )
    ...
    <>
      ...
      {page === 'home' && !search && (
        <footer style={{
          ...
          >
            <div style={{
              width: 28, height: 28, background: 'var(--gold)', borderRadius: 6,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 14, color: '#000', fontWeight: 'bold'
            }}>▶</div>
            <span style={{
              fontFamily: "'Bebas Neue',sans-serif",
              fontSize: 20, color: 'var(--gold)', letterSpacing: 3,
            }}>CHILL CLIPS</span>
          </div>
          ...
          <span style={{ fontSize: 12, color: 'var(--muted)' }}>
            &copy; 2026 Chill Clips &mdash; Built by Oluwasheyi Ojelade
          </span>
        </footer>
      )}
